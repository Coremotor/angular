...
@Component({
  ...
  template: `
    <header class="header">
      <app-toggle (changed)="filter($event)" [buttons]="toggleButtons"></app-toggle>
      <app-cart (clearCart)="clearCart($event)" (removeFromCart)="delete($event)" [inCart]="inCart"></app-cart>
    </header>

    <div class="wrapper">
      <app-product-card
        (addInCart)="addInCart($event)"
        *ngFor="let product of productsList" [product]="product"
      ></app-product-card>
    </div>
...
<!--    </ng-template>-->
  ...
  styles: [
    `
      ...
      .wrapper { display: flex; flex-wrap: wrap; align-content: flex-start; margin: 50px 0 ;}
    `
  ]
})
export class CatalogComponent {
  ...
  addInCart(product: TProduct) {
    ...
    if (item) {
      this.inCart = this.inCart.map((p) => (product.id === p.product.id) ? {count: p.count + 1, product: p.product} : p)
    } else {
  ...
  delete(product: TItemInCart) {
    this.inCart = this.inCart.filter((p) => p.product.id !== product.product.id)
  }
